...
export default function EditProcedurePage({
  ...
  const fetchProcedure = async () => {
    try {
      ...
      if (data.success) {
        const procedure: Procedure = data.data;
        setName(procedure.name);
        setNameEn(procedure.name_en || '');
        setDescription(procedure.description || '');
        setSubcategory(procedure.subcategory || '');
        setDurationMinutes(procedure.duration_minutes);
        setBufferMinutes(procedure.buffer_minutes);
        setBaseCost(procedure.base_cost);
        setRequiresDoctor(procedure.requires_doctor);
        setIsActive(procedure.is_active);

        // 카테고리 설정
        if (CATEGORIES.includes(procedure.category)) {
          setCategory(procedure.category);
        } else {
          setCategory('기타');
          setCustomCategory(procedure.category);
        }

        // 원가 구성요소 설정
        setCostComponents(
          procedure.cost_components.map((c) => ({
            id: c.id,
            cost_type: c.cost_type as CostComponent['cost_type'],
            name: c.name,
            amount: c.amount,
            unit: c.unit || '회당',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        router.push(`/business/procedures/${id}`);
      } else {
        setError(data.error || '시술 수정에 실패했습니다.');
      }
    } catch {
      setError('서버 오류가 발생했습니다.');
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        <Link
          href={`/business/procedures/${id}`}
          className="text-gray-600 hover:text-gray-900"
        >
          ← 시술 상세
        </Link>
        <h1 className="text-3xl font-bold text-gray-900 mt-2">시술 수정</h1>
      </div>
